...
\begin{sol}[\cite{book:UAG} Ex. 2.3.1]\label{ex:UAG_2.3.1}
  ...
  As such $\overline{x}^{\alpha} = x_{1}^{\alpha_{1}} \cdots x_{n}^{\alpha_{n}} > x_{1}^{a}$. if and only if $alpha_{1} > a_{1}$ which is equivalent to saying that $x_{1}^{a_{1}} \mid x_1^{\alpha_{1}} \mid \overline{x}^{\alpha}$
\end{sol}
