...
\begin{sol}[\cite{book:UAG} Ex. 4.3.9]\label{ex:UAG_4.3.9}
  Recall that $f_{i} = F_{i}\eval_{t = 1}$ and that $u = U\eval_{t = 1}$ is a unit in $\Loc_{>}k[\overline{x}]$.
  We also have that ideal membership is invariant up to multiplication by units.
  Then, note that $H\eval_{t = 1} = h = 0$ if and only if $H = 0$.
  These facts imply the statement as well as its converse.
\end{sol}
