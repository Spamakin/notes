  ...
  \begin{enumerate}
    \item Using SageMath, we find the following four solutions:
          \begin{gather*}
            (-1.032426, -1.732051), \\
            (1.032426, 1.732051), \\
            (-0.516213 + 3.130404i, 1.732051), \\
            (0.516213 - 3.130404i,  -1.732051).
          \end{gather*}
    \item In \Cref{fig:4-2-3b_plot}, we see that the points of intersection ``bounce'' which we know geometrically indicates these points have multiplicity > 1.
          \begin{figure}[h]
            \centering
    ...
    \item Using SageMath, we see that a monomial basis of $k[x, y] / I$ is given by $\set{1, x, y, x^{2}, xy, x^{2}y}$.
          ...
          \[
            x^{6} - 18x^{4} + 81x^{2} - 108 = (x^{2} - 12)(x^{2} - 3)^{2}.
          \]
    \clearpage
    \item
  \end{enumerate}
