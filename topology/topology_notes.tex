...
\quest{TODO:\@ Baire Category stuff}
...
\begin{ex}[Sorgenfrey's Half-Open Square Topology, Product of Normal may not be Normal]\
  ...
  \begin{figure}[h]
    ...
    \caption{$S(p, \e)$}
    \label{fig:sorgenfrey_half_open}
  \end{figure}
  ...
  \quest{Something something Baire Category Theorem, I don't get it...}
\end{ex}
...
\begin{pf}
  ...
  By \Cref{lem: urysohn_lemma}, there exists a function $g_{1}\colon X \to \bqty{0, \frac{1}{3}}$ such that
  \[
    g_{1}(x) = \begin{cases}
  ...
  \quest{Clearly the bounds are also correct}.
  ...
  If we can arrange that $g_{1}(x)$ is never $0$ (respectively $1$) if $h \circ f$ is never $0$ (respectively $1$) then $g = h^{-1} \circ g-1$ would be defined and would extend $f$.
  Thus put
  \begin{align*}
  ...
  Then $g_{2}$ is always betweep $g_{1}$ and $\frac{1}{2}$ with $g_{2} \neq g_{1}$ on $C$.
