...
\end{thrm}
\begin{pf}
  \quest{I do not understand this proof at all. Look at Lee ITM}
  Without loss of generality, by paracompactness of $X$ we may assume that $\textbf{U} = \set{U_{\alpha}}_{\alpha \in A}$ is locally finite.
  Consider a family of functions $F = \set{g_{\beta}\colon X \to [0, 1]}_{\beta \in A_{F}}$, where $A_{F} \subseteq A$, such that for $W_{\beta} \defeq \set{x | g_{\beta}(x) > 0}$ we have $\overline{W_{\beta}} \subseteq U_{\beta}$ and $\set{W_{\beta}}_{\beta \in A_{F}} \cup \set{U_{\alpha}}_{\alpha \in A \setminus A_{F}}$ covers $X$.
  \quest{Why does such a family exist?}.
  Let \textbf{F} be a collection of such families ordered by inclusion and is a maximal such collection \quest{what?}.
  Let $G$ be the union of all the families in \textbf{F}.
  We claim that $G$ is an element of \textbf{F}.
  Note that we have that $G = \set{g_{\beta}}_{\beta \in A_{G}}$ and that $A_{G}$ is the union of all the families in \textbf{F}.
  Suppose that $G \notin \textbf{F}$.
  Then there is some $x \in X$ not in any of the $W_{\beta}$ for $\beta \in A_{G}$ or in any of the $U_{\alpha}$ for $\alpha \in A \setminus A_{G}$.
  Let $\set{\alpha_{1}, \ldots, \alpha_{n}}$ be the finite, non-empty set such that $x \in U_{\alpha_{i}}$ for all $i$.
  Then each $\alpha_{i}$ is in $A_{G}$.
  Since \textbf{F} is simply ordered, there is some $F \in \textbf{F}$ such that $\alpha_{i} \in A_{F}$ for all $i$.
  Then $x$ is in some $W_{\alpha_{i}}$ \quest{which is a contradiction}.
  Thus, $G \in \textbf{F}$ as claimed.

  We now show that $A_{G} = A$.
  Suppose now and let $\alpha \in A \setminus A_{G}$.
  Let
  \[
    D \defeq \bigcup_{\beta \in A_{G}} W_{\beta} \cup \bigcup_{\substack{\gamma \in A \setminus A_{G} \\ \gamma \neq \alpha}} U_{\gamma}.
  \]
  Then we have that $X = D \cup U_{\alpha}$ \quest{why}.
  Let $C = X \setminus D$ which is closed and contained in $U_{\alpha}$.
  Since $X$ is normal by \Cref{thrm: paracompact_is_normal}, there exists an open set $V$ such that $C \subseteq V \subseteq \overline{V} \subseteq U_{\alpha}$.
  By \Cref{lem: urysohn_lemma}, there exists a map $g_{\alpha}\colon X \to [0, 1]$ which is $1$ on $C$ and $0$ outside of $V$.
  But $W_{\alpha} \containseq C$ and so $X = D \cup W_{\alpha}$.
  Thus $G \cup \set{g_{\alpha}}$ is a collection of maps satisfying the above conditions and properly containing $G$.
  This contradicts the maximality of \textbf{F} and thus we must have that $A_{G} = A$.

  Now we have a collection of maps $\set{g_{\alpha}}_{\alpha \in A}$ such that all the $W_{\alpha}$ cover $X$ and $\overline{W_{\alpha}} \subseteq U_{\alpha}$.
  Define $g = \Sum_{\alpha} g_{\alpha}$.
  This is well defined by \quest{local finiteness}.
  Note that $g(x) > 0$ for all $x$.
  Let $f_{\alpha} = g_{\alpha} / g$ which yields our \quest{desired partition of unity}.
\end{pf}

We have actually proven more than what was stated.
If the original covering was already locally finite, then we did not need refinement.
Also note that the $W_{\alpha}$ form a covering and $\overline{W_{\alpha}} \subseteq U_{\alpha}$.
Thus we have:
\begin{prop}
  If $X$ is paracompact and $\set{U_{\alpha}}$ is a locally finite open covering of $X$ then there is an open covering $\set{V_{\alpha}}$ such that for each $\alpha$ we have $\overline{V_{\alpha}} \subseteq U_{\alpha}$. \qed
\end{prop}
