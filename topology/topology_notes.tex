...
\begin{pf}
  ...
  By \Cref{prop: homeo_is_open_and_closed_bij}, we have that $f$ is a homeomorphism onto its image.
...
\begin{pf}
  ...
  \begin{enumerate}
  \item[] $1 \iff 2\colon$ See \Cref{thrm: compact_iff_nonempty_intersection}.
  ...
  \item[] $3 \implies 4\colon$ This is clear by \Cref{thrm: net_has_universal_subnet}.
...
\begin{pf}
  ...
  By \Cref{lem: urysohn_lemma} there is a map $k\colon X \to [0, 1]$ which is $0$ on $B$ and $1$ on $F$.
...
\end{prop}
\begin{pf}
  Note that compact spaces are immediately locally compact and by \Cref{thrm: locally_comp_Haus_comp_reg} locally compact Hausdorff spaces are completely regular, so $C$ is completely regular.
...
\begin{pf}
  ...
  Each of the $A_{i}$ are compact and thus normal by \Cref{thrm: compact_haus_is_norm}.
...
\begin{pf}
  \begin{figure}[h]
  \centering
  % https://q.uiver.app/#q=
  \[
    \begin{tikzcd}[every label/.append style={font=\normalsize}]
      A && {A/\sim} \\
      \\
...
\begin{pf}
  ...
  \begin{figure}[h]
    \centering
    % https://q.uiver.app/#q=WzAsNCxbMCwwLCJHIFxcdGltZXMgRyJdLFsyLDAsIkciXSxbMCwyLCJHL0ggXFx0aW1lcyBHL0giXSxbMiwyLCJHL0giXSxbMCwxXSxbMCwyLCJcXHBpIFxcdGltZXMgXFxwaSIsMl0sWzIsM10sWzEsMywiXFxwaSJdXQ==
    \[
      \begin{tikzcd}[every label/.append style={font=\normalsize}]
        {G \times G} && G \\
        \\
...
\begin{ex}[Classical Lie Groups]
  ...
  The set $\text{O}(n)$ is the set of orthogonal real matrices and is a subgroup of $\GL(n, \R)$ and also is a closed subset.
  To see why, consider the map $F\colon \GL(n, \R) \to \GL(n, \R)$ mapping $A \mapsto A A^{t}$.
  Then the preimage of the identity is precisesly the orthogonal matrices.
  Since $\GL(n, \R)$ is a metric space, singletons are closed.
  Thus $\text{O}(n)$ is closed.
  ...
  The quaternionic analogue of the orthogonal and unitary groups is the sympectic group $\Sp(n)$.
  Its elements are quaternionic matrices $A$ such that $A A^{*} = I$ where $A^{*}$ is the quaternionic conjugate transpose of $A$.
