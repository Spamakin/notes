...
\begin{ex}[Constant Map $\simeq 1_{X}$]
  ...
  By \Cref{prop: contractable_iff_single_point} this means that $\R^{n}$ is contractible.
...
\begin{align*}
  ...
  \SU(n) &= \set{A \in \text{U}(n) | \det(A) = 1} = \text{the special orthogonal group.}
\end{align*}
...
\begin{align*}
  \SO(n) / \SO(n - 1) & \textbf{S}^{n - 1} & \text{for } n \geq 2; \text{ and } \\
  \SU(n) / \SU(n - 1) & \textbf{S}^{2n - 1} & \text{for } n \geq 2.
\end{align*}
