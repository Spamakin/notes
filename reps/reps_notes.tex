...
\begin{pf}
  ...
  Then by \Cref{er:Serre_2_2}, $s \in G$, $\chi(s) = 1$ and so by \quest{Exercise 2.5} we have that $(\chi \mid 1) = \frac{1}{\abs{G}} \sum \chi(s) = 1$.
  Note that the unit representation is irreducible, and so by \Cref{thrm:scalar_counts_num_iso} we have that the number of times $\rho$ contains the unit representation is equal to $(\chi \mid 1) = 1$ as desired.
